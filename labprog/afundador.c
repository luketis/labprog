...
void afunda_embarcacao(coord coordenadas[3], Map mapa) {
	int i;
	for(i = 0; i < 3; i++) {
		if((coordenadas[i]).x != -1) {
			if((mapa->map)[(coordenadas[i]).x][(coordenadas[i]).y] == 'D')
				 afunda_destroyer(coordenadas[i], mapa);

			else if((mapa->map)[(coordenadas[i]).x][(coordenadas[i]).y] == 'C')
				 afunda_cruzador(coordenadas[i], mapa);

			else if((mapa->map)[(coordenadas[i]).x][(coordenadas[i]).y] == 'P')
				 afunda_porta_aviao(coordenadas[i], mapa);

			else if((mapa->map)[(coordenadas[i]).x][(coordenadas[i]).y] == 'H')
				 afunda_hidro_aviao(coordenadas[i], mapa);
		}
	}
